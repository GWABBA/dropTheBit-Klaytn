...
export const ProgramWEB = async () => {
    ...
    const ecsTaskExecutionWithSecretsRole = new aws.iam.Role(`${PREFIX.PROJECT}-ecs-task-execution`, {
        name: `${PREFIX.PROJECT}-ecsTaskExecution`,
    ...
    const taskDefinition = pulumi.all([ecrRepository.repositoryUrl])
        .apply(([repositoryUrl]) => {
            return new aws.ecs.TaskDefinition(`${PREFIX.PROJECT}-task-definition`, {
                    ...
                    [
                        {
                            ...
                            "portMappings": [
                                {
                                    "containerPort": Number(process.env.WEB_SERVICE_CONTAINER_PORT),
                                    "hostPort": Number(process.env.WEB_SERVICE_CONTAINER_PORT),
                                    "protocol": "tcp"
                                }
                            ],
                            "privileged": null,
                ...
                cpu: process.env.WEB_TASK_DEFINITION_CPU,
                memory: process.env.WEB_TASK_DEFINITION_MEMORY,
                runtimePlatform: {
    ...
    //create security group for ecs service which is WEB_SERVICE_CONTAINER_PORT
    ...
    const service = new aws.ecs.Service(`${PREFIX.PROJECT}-ecs-service`, {
        ...
        desiredCount: Number(process.env.WEB_SERVICE_DESIRED_COUNT),
        deploymentMaximumPercent: Number(process.env.WEB_SERVICE_MAXIMUM_PERCENT),
        deploymentMinimumHealthyPercent: Number(process.env.WEB_SERVICE_MINIMUM_HEALTHY_PERCENT),
        loadBalancers: [
            {
                targetGroupArn: targetGroup.arn,
